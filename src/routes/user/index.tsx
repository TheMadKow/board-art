    ...
    <div>
      ...
      <div className={styles.section}>
        ...
        <div className={styles.syncRow}>
          <button className={styles.btnSync} onClick={handleSync} disabled={!canSync}>
            {syncing ? "Syncing…" : "Sync Library"}
          </button>
          {username.trim() === "" && <span className={styles.hint}>Save a BGG username first</span>}
          {syncing && (
        ...
        {syncResult && (
          <div className={styles.resultBox}>
            ...
            <div className={styles.resultStat}>
              <strong className={styles.added}>{syncResult.added.length}</strong> game
              {syncResult.added.length !== 1 ? "s" : ""} added
            </div>
            {syncResult.added.length > 0 && (
              <ul className={styles.gameList}>
                {syncResult.added.map((g) => (
                  <li key={g.bggId}>{g.title}</li>
                ))}
              </ul>
            )}
            <div className={styles.resultStat} style={{ marginTop: 10 }}>
              <strong className={styles.removed}>{syncResult.removed.length}</strong> game
              {syncResult.removed.length !== 1 ? "s" : ""} marked as removed
            </div>
            {syncResult.removed.length > 0 && (
              <ul className={styles.gameList}>
                {syncResult.removed.map((g) => (
                  <li key={g.title}>{g.title}</li>
                ))}
              </ul>
            )}
            <div className={styles.resultStat} style={{ marginTop: 10 }}>
              <strong>{syncResult.unchanged}</strong> game{syncResult.unchanged !== 1 ? "s" : ""}{" "}
              unchanged
            </div>
          </div>
        )}
