...
function seed(db: Database.Database) {
  ...
  const seedAll = db.transaction(() => {
    ...
    insertPrint.run(
      "p3",
      "Terraforming Mars Player Trays",
      "g3",
      "printing",
      "Currently on 3rd set",
    );
    insertPrint.run("p4", "Root Faction Tokens", "g4", "planned", null);
    insertPrint.run(
      "p5",
      "Spirit Island Custom Island Board",
      "g5",
      "planned",
      "Need to find a good model",
    );
    ...
    insertPrint.run(
      ...
      null,
      "planned",
      "Universal design for medium boxes",
    );
  });
...
try {
  db.exec("ALTER TABLE games ADD COLUMN removed INTEGER NOT NULL DEFAULT 0");
} catch {
  /* column exists */
}
