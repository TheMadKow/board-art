...
export const getSettings = createServerFn({ method: 'GET' }).handler((): Settings => {
  const row = db.prepare('SELECT value FROM settings WHERE key = ?').get('bgg_username') as { value: string } | null
...
async function fetchBggCollection(username: string): Promise<Array<{ bggId: string; title: string }>> {
  const url =
...
type GameRecord = { id: string; title: string; bgg_id: string | null; owned: number; removed: number }
